...
def put_piece(board, max_rows, column, player):
    ...
    for i in reversed(board):
        if i[column] == '-':
            row_lst = replace_elem(i, column, player)
            new_board = replace_elem(board, row, row_lst)
            break;
        row -= 1
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    return put_piece(board, max_rows, col, player) if 0 <= col < max_cols else (-1, board)
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for i in range(max_cols):
        row_check += get_piece(board, row, i)
    if player * num_connect in row_check:
        return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    col_check = ""
    for i in range(max_rows):
        col_check += get_piece(board, i, col)
    if player * num_connect in col_check:
        return True
    return False
