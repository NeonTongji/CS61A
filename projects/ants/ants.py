...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if self.change_direction:
            if destination.entrance:  # destination can't be hive
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while check_place != hive and step < self.max_range + 1:
            bee = random_or_none(check_place.bees)
            if bee:
                return bee
            else:
                check_place = check_place.entrance
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.TrueQueen:
            ...
            buffer_place = self.place.exit
            ants = []
            
            while buffer_place.exit: 
                if buffer_place.ant:
                    ants.append(buffer_place.ant)
                    if buffer_place.ant.is_container and buffer_place.ant.contained_ant:
                        ants.append(buffer_place.ant.contained_ant)
                buffer_place = buffer_place.exit
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        while check_place != hive:
            if check_place.ant and check_place.ant != self:
                insects[check_place.ant] = distance 
            if check_place.bees:
