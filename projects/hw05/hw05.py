...
def balanced(m):
    ...
    if is_mobile(end(left(m))):
        left_b = balanced(end(left(m)))
    if is_mobile(end(right(m))):
        right_b = balanced(end(right(m)))
...
def totals_tree(m):
    ...
    if is_mobile(end(left(m))):
        ...
        left_t = tree(size(end(left(m))))
    if is_mobile(end(right(m))):
        right_t = totals_tree(end(right(m)))
    else:
        right_t = tree(size(end(right(m))))
...
def make_counter():
    ...
    "*** YOUR CODE HERE ***"
    counter_dic = {}
    def counter_helper(s):
        if s in counter_dic.keys():
            counter_dic[s] += 1
        else:
            counter_dic[s] = 1
        return counter_dic[s]
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password):
        ...
        if password in account.keys():
            if amount > account[password]:
